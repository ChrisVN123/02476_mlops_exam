...
from src.exam_project.data import find_sparse_rows_by_initials, load_and_preprocess_data # type: ignore
...
def predict(model_path: str = "models/model.onnx", initials: str = "APPL"):
    try:
        ...
        labels_array = labels.to_numpy().flatten()
        prediction = labels_array[np.argmax(out)]
        correct = labels_array[y]
